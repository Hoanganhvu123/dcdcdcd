...
class NerdctlSandboxSession(SandboxSession):
    ...
    async def start(self) -> bool:
        try:
            ...
            # 网络隔离 (与 docker/podman 一致)
            if getattr(self.config, "network_disabled", False):
                if str(self.config.language).endswith("-vnc"):
                    # VNC sessions must publish 5900/6080, which "--network none"
                    # makes impossible. Refuse rather than quietly running the
                    # container with a network the caller asked us to remove.
                    raise ValueError(
                        "network_disabled is incompatible with VNC sessions "
                        f"(language={self.config.language!r}), which require "
                        "published ports"
                    )
                args += ["--network", "none"]
