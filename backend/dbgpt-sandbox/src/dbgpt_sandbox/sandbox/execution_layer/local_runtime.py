...
from .kernel_runtime import KernelSandboxRunner
from .utils import PathUtils, ProcessManager, SecurityUtils

#: Host environment variables that may be inherited by sandboxed code.
#:
#: The local runtime executes untrusted code as a plain child process, so
#: anything left in its environment is readable by that code. Passing the host
#: environment through wholesale handed it every server secret — API keys,
#: Langfuse credentials, database URLs. The container runtimes never had this
#: problem: they start from a clean image environment and inject only
#: ``SessionConfig.environment_vars``. This allowlist gives the local runtime the
#: same property, keeping just the variables a child process needs to start and
#: the non-secret toolchain roots the supported languages resolve against.
_ENV_ALLOWLIST = frozenset(
    {
        ...
        "USER",
        "LOGNAME",
        # Windows process basics — a child cannot start without these
        "SYSTEMROOT",
        "SYSTEMDRIVE",
        "WINDIR",
        "COMSPEC",
        ...
        # Non-secret toolchain roots for the supported languages
        "JAVA_HOME",
        "GOPATH",
        "GOROOT",
        "GOCACHE",
...
def build_child_env(environment_vars: Dict[str, str]) -> Dict[str, str]:
    """Environment for a sandboxed child: allowlisted host vars plus session vars.

    Comparison is upper-cased because Windows normalizes environment variable
    names to upper case, so a case-sensitive test would drop ``Path`` there and
    leave the child unable to resolve its interpreter.

    ``environment_vars`` are applied last and are never filtered: they were
    supplied deliberately for this sandbox by the caller.
    """
...
class LocalSandboxSession(SandboxSession):
    ...
    async def start(self) -> bool:
        ...
        try:
            if getattr(self.config, "network_disabled", False):
                # The container runtimes enforce this with "--network none".
                # A plain child process shares the host network stack and there
                # is no portable way to take it away, so honour the request by
                # refusing to start rather than running with full network access
                # while reporting that it was disabled.
                raise ValueError(
                    "network_disabled is not enforceable by the local runtime; "
                    "use a container runtime (docker/podman/nerdctl) for "
    ...
    def _setup_environment(self):
        """设置执行环境"""
        # NOTE: session environment variables are deliberately NOT written into
        # os.environ here. That mutated the host process globally, leaking one
        # sandbox session's configuration into the server and into every other
        # session. They are passed per-process instead, via build_child_env().
    ...
    async def _run_with_limits(self, command: List[str]) -> Dict[str, Any]:
        """在资源限制下运行命令"""
        process = None
        # Kernel confinement, when the deployment asks for it. Default policy is
        # "off", so this is a passthrough until DBGPT_KERNEL_SANDBOX is set; on
        # "require" without a backend it raises and no child is ever spawned.
        confined = KernelSandboxRunner(self.work_dir).wrap(command)
        try:
            # 启动进程
            process = await asyncio.create_subprocess_exec(
                *confined.argv,
                ...
                cwd=self.work_dir,
                env=build_child_env(self.config.environment_vars),
            )
