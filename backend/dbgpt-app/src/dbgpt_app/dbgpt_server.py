...
def mount_routers(app: FastAPI):
    ...
    from dbgpt_app.openapi.api_v1.analyst_api import router as analyst_api
    from dbgpt_app.openapi.api_v1.replay_api import router as replay_api
    ...
    if os.getenv("ANALYST_AGENT_ENABLED", "true").lower() == "true":
        app.include_router(analyst_api)
    app.include_router(replay_api)
    ...
    async def health_check():
        """Health check endpoint confirming server readiness."""
        return {
            "status": "ok",
            "service": "dbgpt",
            "version": version,
...
def initialize_app(param: ApplicationConfig, args: List[str] = None):
    ...
    # Bootstrap Analyst Agent metadata tables on server startup (Tasks U & V)
    if os.getenv("ANALYST_AGENT_ENABLED", "true").lower() == "true":
        try:
            from dbgpt_analyst.libs.bi.connections import bootstrap_analyst_metadata
            bootstrap_analyst_metadata()
            logger.info("Successfully bootstrapped DB-GPT Analyst metadata on server startup")
        except Exception as e:
            logger.error(f"Failed to bootstrap Analyst metadata on server startup: {e}", exc_info=True)
