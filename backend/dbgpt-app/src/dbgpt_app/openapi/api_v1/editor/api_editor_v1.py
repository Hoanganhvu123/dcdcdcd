...
def sanitize_sql(sql: str, db_type: str = None) -> Tuple[bool, str, dict]:
    ...
    # Allow SELECT, PRAGMA, CREATE TABLE, INSERT, UPDATE, and DELETE operations
    # We're no longer restricting to read-only operations
    allowed_operations = re.match(
        r"(?i)^\s*(SELECT|PRAGMA|CREATE\s+TABLE|INSERT\s+INTO|UPDATE|DELETE\s+FROM|ALTER\s+TABLE)\b",
        sql,
    )
    if not allowed_operations:
        return (
            False,
            "Operation not supported. Only SELECT, PRAGMA, CREATE TABLE, INSERT, UPDATE, "
            "DELETE and ALTER TABLE operations are allowed",
            {},
        )

    # If the statement is a PRAGMA statement, do not parameterize string literals
    # because SQLite PRAGMAs do not accept bound parameters (e.g., PRAGMA table_info(?))
