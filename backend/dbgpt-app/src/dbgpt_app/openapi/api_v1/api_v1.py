...
async def chat_completions(
    ...
    try:
        ...
        logger.exception(f"Chat Exception!{dialogue}")
        ...
        async for chunk in chat.stream_call(
            ...
            if openai_format:
                ...
                if isinstance(chunk, str):
                    text = chunk
                elif isinstance(chunk, ModelOutput):
                    output: ModelOutput = chunk
                    if output.has_text:
                        text = output.text
                    if output.has_thinking:
                        think_text = output.thinking_text
                    output_usage = output.usage
                elif hasattr(chunk, "has_text"):
                    if chunk.has_text:
                        text = chunk.text
                    if hasattr(chunk, "has_thinking") and chunk.has_thinking:
                        think_text = chunk.thinking_text
                    output_usage = getattr(chunk, "usage", None)
                else:
                    text = str(chunk)
