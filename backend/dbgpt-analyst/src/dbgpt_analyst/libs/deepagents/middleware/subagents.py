...
def _create_task_tool(
    *,
    ...
    def task(
        ...
        # Isolate sub-agent thread state to avoid parent thread checkpoint corruption
        subagent_config = runtime.config.copy() if runtime.config else {}
        if config:
            subagent_config["callbacks"] = config.get("callbacks")
            subagent_config["tags"] = (subagent_config.get("tags") or []) + (config.get("tags") or [])
            if "configurable" in config:
                if "configurable" not in subagent_config:
                    subagent_config["configurable"] = {}
                for k, v in config["configurable"].items():
                    if k not in subagent_config["configurable"]:
                        subagent_config["configurable"][k] = v

        if "configurable" in subagent_config:
            subagent_config["configurable"] = subagent_config["configurable"].copy()
            parent_thread_id = subagent_config["configurable"].get("thread_id", "default")
            subagent_config["configurable"]["thread_id"] = f"{parent_thread_id}_sub_{runtime.tool_call_id}"
            subagent_config["configurable"].pop("checkpoint_id", None)
            if "checkpoint_ns" in subagent_config["configurable"]:
                subagent_config["configurable"]["checkpoint_ns"] = f"{subagent_config['configurable']['checkpoint_ns']}.sub_{runtime.tool_call_id}"
    ...
    async def atask(
        ...
        # Stream the subagent's own run instead of blocking on a single
        # .ainvoke(), and re-dispatch every event under "subgraph_stream" —
        # same event shape main_graph.py's run_domain() uses for its own
        # domain subgraphs. Without this, callers that only watch custom
        # events (e.g. the SSE layer in api/ai_analytic/streamer.py) saw
        # nothing at all for a delegated subagent until it fully finished:
        # no intermediate tool calls, no token-by-token answer, no way to
        # tell it apart from a hung call.
        final_state: dict[str, Any] | None = None
        try:
            async for kind, payload in subagent.astream(
                subagent_state,
                ...
                if kind == "values":
                    ...
                    emit_custom(
                        "subgraph_stream",
                        {"kind": kind, "payload": payload, "subagent_type": subagent_type, "subgraph_name": subagent_type},
                    )
        except (TypeError, AttributeError):
            # Not every subagent Runnable is a LangGraph graph with a
            # .astream(stream_mode=[...]) signature (e.g. a plain chain) —
            # fall back to the original blocking call for those.
            final_state = await subagent.ainvoke(subagent_state, config=subagent_config)
