"""subgraphs/sql_agent_subgraph.py — SQL Agent Subgraph (Real StateGraph).

Wraps the SQL chain (table_selector → resolve_schema → explore → generate_sql
→ validate → execute_sql → critic) into a proper LangGraph StateGraph.

All retry logic stays INSIDE this subgraph. Only returns to main_graph when
critic says "ok" (no retry needed).

Uses MainAgentState (shared) for now — Phase 2 will add proper state isolation
via SQLAgentState + adapter pattern.
"""
