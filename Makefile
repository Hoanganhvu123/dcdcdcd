...
ifeq ($(OS),Windows_NT)
dev:
	@echo "🚀 Launching DB-GPT Backend & Frontend..."
	powershell -Command "Start-Process powershell -ArgumentList '-NoExit', '-Command', 'cd d:\DB-GPT; uv run dbgpt start webserver'; Start-Process powershell -ArgumentList '-NoExit', '-Command', 'cd d:\DB-GPT\frontend; npm run dev'"
else
dev:
	@echo "🚀 Launching DB-GPT Backend & Frontend..."
	(uv run dbgpt start webserver) & (cd frontend && npm run dev)
endif
