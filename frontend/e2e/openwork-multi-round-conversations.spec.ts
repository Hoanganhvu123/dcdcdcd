...
class StatefulBackendHarness {
  ...
  constructor() {
    ...
    const seedId = 'e2e-seed-conv-00000000-0000-4000-8000-000000000001';
  ...
  async attachToPage(page: Page) {
    ...
    await page.route('**/api/v1/model/types', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ success: true, data: ['deepseek-v4-flash'] }),
      });
    });
...
test.describe('16 - OpenWork Multi-Round Conversations & Skills System E2E Suite', () => {
  ...
  for (const domain of TEST_DOMAINS) {
    test(`16.${domain.domainIndex} - Multi-Round Conversation (${domain.domainName}): verify UUID in URL, streaming response, follow-up persistence, and refresh hydration`, async ({
      ...
      expect(UUID_V4_REGEX.test(convId!)).toBe(true);

      // Wait for streaming to complete (Stop button reverts to Run task)
      await expect(page.locator('button.chat-input-submit')).toContainText('Run task', { timeout: 15000 });
      ...
      fs.copyFileSync(screenshotPathR1, frontendScreenshotPathR1);
      ...
      fs.copyFileSync(screenshotPathR2, frontendScreenshotPathR2);
  ...
  test('16.11 - Sidebar Conversation List: fetches and renders real conversations from GET /api/v1/analyst/conversations', async ({
    ...
    fs.copyFileSync(screenshotPath, path.join(FRONTEND_SCREENSHOT_DIR, 'sidebar_conversations_list.png'));
  ...
  test('16.12 - Skills System: Settings modal skill toggles, Database OFF removes sql_query tool, and persists across refresh', async ({
    ...
    fs.copyFileSync(screenshotSkillsOff, path.join(FRONTEND_SCREENSHOT_DIR, 'skills_modal_database_off.png'));
    ...
    await page.locator('button.chat-input-submit').click();
