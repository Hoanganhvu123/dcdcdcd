...
import { MessagePart, ReasoningPart, ToolPart, ToolStatus } from '@/components/chat/content/OpenCodeSessionTurn';
...
export interface SSEStepOutputEvent {
  type: 'step.output';
  step: number;
  detail: string;
}
...
export interface SSEStepChunkEvent {
  type: 'step.chunk';
  id: string;
  output_type: 'thought' | 'thinking' | 'text' | 'chart' | 'code' | 'markdown' | string;
  content: any;
}
