  ...
  return (
    <div className='flex flex-col h-full w-full dark:bg-gradient-dark bg-gradient-light'>
      <div className='flex items-center justify-between px-6 py-5 border-b border-border'>
        <h1 className='text-xl font-semibold text-foreground'>{t('all_tasks')}</h1>
        <div className='flex items-center gap-3'>
          <Input
            variant='filled'
            prefix={<Search strokeWidth={1.5} />}
            placeholder={t('search_conversations')}
            onChange={e => handleSearch(e.target.value)}
            onClear={() => setSearchKeyword('')}
            allowClear
            className='w-[230px] h-[36px] border border-border bg-card'
          />
          <span className='text-sm text-muted-foreground whitespace-nowrap'>
            {totalRef.current ? t('total_count', { count: totalRef.current.total_count }) : ''}
          </span>
        </div>
      </div>
            ...
            <div className='space-y-1'>
              {filteredList.map(conv => (
                <div
                  ...
                  className='group flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-colors hover:bg-white dark:hover:bg-accent hover:shadow-sm'
                >
                  <div className='flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-muted'>
                    <MessageSquare strokeWidth={1.5} className='text-muted-foreground text-sm' />
                  </div>

                  <div className='flex-1 min-w-0'>
                    <div className='text-sm font-medium text-foreground truncate leading-5'>
                      {getTitle(conv)}
                    </div>
                    {conv.gmt_created && (
                      <div className='text-xs text-muted-foreground mt-0.5'>{formatTime(conv.gmt_created)}</div>
                    )}
                  </div>
                  ...
                  >
                    <Tooltip title={t('Delete')}>
                      ...
                      >
                        <Trash2 strokeWidth={1.5} className='text-muted-foreground hover:text-foreground transition-colors' />
                      </div>
                    </Tooltip>
                  </Popconfirm>
                </div>
      ...
      {(totalRef.current?.total_count ?? 0) > PAGE_SIZE && (
        <div className='flex justify-end px-6 py-4 border-t border-border'>
          <Pagination
            ...
            showTotal={total => t('total_count', { count: total })}
