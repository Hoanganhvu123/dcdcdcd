/* eslint-disable */
/* eslint-disable no-danger, prefer-dynamic-import, jsx-no-constructed-context-values, js-flatmap-filter */
"use client";
...
import { sanitizeCssColor, sanitizeCssValue } from "@/lib/security/sanitizer";
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const safeId = sanitizeCssValue(id).replace(/[^a-zA-Z0-9_-]/g, "");
...
${colorConfig
  .map(([key, itemConfig]) => {
    ...
    const safeKey = sanitizeCssValue(key).replace(/[^a-zA-Z0-9_-]/g, "");
