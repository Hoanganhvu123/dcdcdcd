...
import { getUserId, safeJsonParse } from '@/utils';
...
function getDefaultTheme(): ThemeMode {
  ...
  // Light is the product default. It used to follow `prefers-color-scheme`, which meant
  // anyone on a dark OS landed on the dark shell even though the design system (shadcn
  // tokens in globals.css) is a light warm canvas palette that only dark: variants override.
  return 'light';
}
...
const ChatContextProvider = ({ children }: { children: React.ReactElement }) => {
  ...
  const { run: queryAdminListRun } = useRequest(
    async () => {
      // queryAdminList is a local stub returning an array, not an HTTP call, so it must not be
      // wrapped in apiInterceptors (which expects an axios promise).
  ...
  useEffect(() => {
    ...
    const fallbackDialogInfo = {
      chat_scene: '',
      app_code: '',
    };
    const dialogInfo = typeof window !== 'undefined'
      ? safeJsonParse(localStorage.getItem('cur_dialog_info'), fallbackDialogInfo)
      : fallbackDialogInfo;
